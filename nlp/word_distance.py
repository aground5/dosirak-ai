...
def min_distance_in_list(target: str, word_list: list):
    distances = [jamo_levenshtein(target, word) for word in word_list]
    min_distance = min(distances)
    min_words = [word_list[i] for i in range(len(distances)) if distances[i] == min_distance]
    return min_words, min_distance


if __name__ == "__main__":
    # "가나다라"와 "가마바라"의 거리
    print(jamo_levenshtein("가나다라", "가마바라"))

    # 실행 예
    samples = ["장진수", "장진순", "장짐수", "장진경", "장진욱"]
    base = samples[0]
    r = sorted(samples, key=lambda n: jamo_levenshtein(base, n))
    for n in r:
        print(jamo_levenshtein(base, n), n)
